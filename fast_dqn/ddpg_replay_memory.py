...
# TODO: Kevin note: I copied out the replay memory because there were some
# updates that would seem to hurt fast-dqn (for one, action spaces differing
# like being vectors instead of int32s), so I moved it out for now. We can merge
# back in later. It might just be a simple change, but wanted to be safe for now.
class DDPGReplayMemory:
...
class ScalarReplayMemory:
    ...
    def _allocate(self, observation, action):
        ...
        self.actions = np.empty([self._capacity, *action.shape], dtype=np.float32)
    ...
    def save(self, state, action, reward, done):
        # NOTE: Assumes observations are stacked on last axis to form states and contain RGB images
        observation = state[..., -3:]
        ...
        if not self._allocated:
            ...
            # NOTE: For RGB
            self._history_len = state.shape[-1] // 3
